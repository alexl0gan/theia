...
to overcome shortcomings of the conventional approach:
- tests are written against the application APIs
ensuring that completness and timing of APIs are tested;
- tests are executed within the application process
ensuring their speed and robustness.

## Testing principles
- **Information Hidding**: API should provide the application object model
hidding DOM/CSS implementation details behind.
Test against the application API, not implementation details, like DOM/CSS.
- **Completness**: API should be complete, i.e. stateful service
...
Watch out for complex functions which do everything in tests and implementation.
- **Convenience**: API should provide convenient functions
for typical complex tasks, such functions although
should not be complex, but broken down to follow the extensibility principle.
...
a function should resolve when an editor actually focused.
...
One can acess a module with `window.theia.moduleName`, where `moduleName`
...
Testing framework as well injects `require` function to look up modules.
It can be useful with enabled typescript checks for js files to write statically checked code.

Importing symbols from exposed module is not enough,
one has to access their implementations from the appication container.
...
- **Information Hidding**: the object model (EditorManager) is provided to access editors, no DOM/CSS are used.
- **Completness**: API provides a way to access existing editors and open new editors,
there are also events notifying when a new editor get created or closed. More test can be added to test it.
- **Extensibility**: EditorManager is not implementing everything but reuse WidgetOpenHandler and WidgetManager.
...
but such logic is already encapsulated in `EditorManager.open` which allows to keep a test simple.
- **Robustness**: Test relies on `EditorManager.open` to resolve when a widget is actually revealed.
...
This command starts the browser example application and run tests from `examples/api-tests` against it.
